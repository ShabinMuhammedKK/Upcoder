...
export class CreateUsersData implements userUseCaseInterf<UserEntity, UserEntity> {
  ...
  async registerNewUser(formData: UserEntity,otp: number): Promise<UserEntity> {
    ...
    try {
      ...
      if (enteredOTP !== Number(generateOtp?.otp)) {
        ...
        if (createNewUser) {
          ...
          const createToken = jwt.sign(tokenData, process.env.JWT_SECRET_KEY,{ expiresIn: '1h' });
  ...
  async editUserProfile(userData:UserEntity):Promise<boolean>{
    try {
