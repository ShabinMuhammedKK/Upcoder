...
const UpdateUserData: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        const response = await axios.post('http://localhost:3000/auth/user/updateuser', formData);
        if (response) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-center text-black">Edit Profile</h2>
